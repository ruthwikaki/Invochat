-- Fixes missing RPC Functions for AI Features
-- This script creates the missing functions that AI tools expect, using the correct table structure.
    ...
    RETURN (
        ...
        FROM (
            SELECT
                ...
                SUM(oli.quantity) as total_sold,
                SUM(oli.quantity * oli.price) as total_revenue,
                SUM(oli.quantity * COALESCE(oli.cost_at_time, pv.cost, 0)) as total_cost,
                ...
                SUM(oli.quantity * oli.price) - SUM(oli.quantity * COALESCE(oli.cost_at_time, pv.cost, 0)) as gross_profit
            FROM public.order_line_items oli
            JOIN public.orders o ON oli.order_id = o.id
            LEFT JOIN public.product_variants pv ON pv.sku = oli.sku AND pv.company_id = oli.company_id
            WHERE oli.company_id = p_company_id
            AND o.created_at >= NOW() - INTERVAL '90 days'
            AND o.financial_status = 'paid'
            GROUP BY oli.sku, oli.product_name
    ...
    RETURN (
        ...
        FROM (
            SELECT
                ...
                SUM(oli.quantity * oli.price) as revenue,
                SUM(oli.quantity * COALESCE(oli.cost_at_time, pv.cost, 0)) as cost,
                ROUND(((SUM(oli.quantity * oli.price) - SUM(oli.quantity * COALESCE(oli.cost_at_time, pv.cost, 0))) / NULLIF(SUM(oli.quantity * oli.price), 0) * 100)::numeric, 2) as margin_percentage
            FROM public.order_line_items oli
            JOIN public.orders o ON oli.order_id = o.id
            LEFT JOIN public.product_variants pv ON pv.sku = oli.sku AND pv.company_id = oli.company_id
...
BEGIN
    RETURN (
        ...
        FROM (
            ...
            LEFT JOIN public.product_variants pv ON pv.sku = weekly_data.sku AND pv.company_id = p_company_id
...
BEGIN
    RETURN (
        ...
        FROM (
            ...
            LEFT JOIN public.product_variants pv ON pv.sku = daily_data.sku AND pv.company_id = p_company_id
