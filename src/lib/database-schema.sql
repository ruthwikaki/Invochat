...
-- Step 2: Table Definitions (All tables created first)
---------------------------------------------------------
...
-- Step 5: Functions (All functions created after tables)
---------------------------------------------------------

-- Drop existing functions before creating new ones to avoid conflicts
...
CREATE OR REPLACE FUNCTION public.batch_upsert_with_transaction(p_table_name text, p_records jsonb, p_conflict_columns text[])
...
RETURNS TABLE(category text) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
...
BEGIN
    ...
    -- Low Stock Alerts
    SELECT
        'low_stock'::text, p.id, p.name::text, i.quantity, i.reorder_point, i.last_sold_date,
        (i.quantity * i.cost)::numeric AS value, NULL::numeric
    ...
    -- Dead Stock Alerts
    SELECT
        'dead_stock'::text, p.id, p.name::text, i.quantity, i.reorder_point, i.last_sold_date,
        (i.quantity * i.cost)::numeric, NULL::numeric
    FROM public.inventory i
    JOIN public.products p ON i.product_id = p.id AND i.company_id = p.company_id
    ...
    SELECT
        'predictive'::text, p.id, p.name::text, i.quantity, i.reorder_point, i.last_sold_date,
        (i.quantity * i.cost)::numeric, (i.quantity / sv.daily_sales)
    FROM public.inventory i
    JOIN public.products p ON i.product_id = p.id AND i.company_id = p.company_id
    JOIN (
        SELECT si.product_id, SUM(si.quantity)::numeric / p_fast_moving_days as daily_sales
    ...
    ) sv ON i.product_id = sv.product_id
...
BEGIN
    RETURN QUERY
    WITH daily_stats AS (
        SELECT s.created_at::date AS stat_date, SUM(s.total_amount) AS revenue, COUNT(DISTINCT s.customer_email) AS customers
        FROM public.sales s WHERE s.company_id = p_company_id AND s.created_at >= NOW() - INTERVAL '30 days'
    ...
    anomalies AS (
        SELECT ds.stat_date, ds.revenue, ds.customers, a.avg_revenue, a.avg_customers,
...
RETURNS TABLE(product_id uuid) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
...
CREATE OR REPLACE FUNCTION public.record_sale_transaction(p_company_id uuid, p_user_id uuid, p_sale_items jsonb, p_customer_name text DEFAULT NULL, p_customer_email text DEFAULT NULL, p_payment_method text DEFAULT 'other', p_notes text DEFAULT NULL, p_external_id text DEFAULT NULL)
RETURNS public.sales LANGUAGE plpgsql AS $$
DECLARE
    new_sale public.sales; item_record record; total_amount bigint := 0; tax_amount bigint := 0;
    company_tax_rate numeric; current_inventory record; min_stock_level integer; v_product_id uuid;
BEGIN
    FOR item_record IN SELECT * FROM jsonb_to_recordset(p_sale_items) AS x(product_id uuid, quantity int) LOOP
        ...
        IF NOT FOUND OR current_inventory.quantity < item_record.quantity THEN RAISE EXCEPTION 'Insufficient stock for product_id: %. Available: %, Requested: %', v_product_id, COALESCE(current_inventory.quantity, 0), item_record.quantity; END IF;
        ...
        IF min_stock_level IS NOT NULL AND (current_inventory.quantity - item_record.quantity) < min_stock_level THEN RAISE EXCEPTION 'This sale would bring stock for product_id % below its minimum level of %.', v_product_id, min_stock_level; END IF;
    END LOOP;
    ...
    FOR item_record IN SELECT * FROM jsonb_to_recordset(p_sale_items) AS x(product_id uuid, quantity int, unit_price bigint, cost_at_time bigint) LOOP
        ...
        VALUES (new_sale.id, p_company_id, item_record.product_id, item_record.quantity, item_record.unit_price, COALESCE(item_record.cost_at_time, 0));
    END LOOP;
    RETURN new_sale;
END;
$$;
...
RETURNS void AS $$ BEGIN INSERT INTO public.audit_log (user_id, company_id, action, details) VALUES (p_user_id, p_company_id, p_action, p_details); END; $$
...
RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN DELETE FROM public.inventory WHERE deleted_at < NOW() - (p_retention_days || ' days')::interval; DELETE FROM public.customers WHERE deleted_at < NOW() - (p_retention_days || ' days')::interval; END; $$;
...
BEGIN
    ...
    VALUES (p_company_id, p_product_id, p_from_location_id, p_user_id, 'transfer_out', -p_quantity, (SELECT quantity FROM public.inventory WHERE product_id = p_product_id AND location_id = p_from_location_id), p_notes),
...
RETURNS TABLE(product_name text, sku text, days_since_last_sale integer, quantity integer, total_value bigint) LANGUAGE plpgsql AS $$
BEGIN
    RETURN QUERY
    SELECT p.name as product_name, p.sku, EXTRACT(DAY FROM (NOW() - COALESCE(i.last_sold_date, i.created_at)))::integer as days_since_last_sale, i.quantity, (i.quantity * i.cost)::bigint as total_value
