...
-- Create new policies
CREATE POLICY "Users can access their company data" ON public.orders
    FOR ALL 
...
CREATE POLICY "Users can access their company data" ON public.product_variants
    FOR ALL 
    USING (company_id = auth.company_id());

CREATE POLICY "Users can access their company data" ON public.order_line_items
    FOR ALL 
    USING (company_id = auth.company_id());
