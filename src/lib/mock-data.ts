...
const generateMockData = () => {
    ...
    const mockTransactions: Transaction[] = Array.from({ length: 180 }).map((_, i) => {
    ...
    const mockAlerts: Alert[] = [
      ...
      { id: 'ALE003', item: 'Obsolete Part X', message: 'Item has not sold in over 90 days.', date: '2023-10-24', resolved: true, type: 'Dead Stock' },
    ];
    
    const mockInventoryValueByCategory: InventoryValueByCategory[] = Object.entries(
      mockProducts.reduce(
        (acc, p) => {
    ...
    const mockDeadStock = deadStockProducts.map(p => ({
        id: p.sku,
        name: p.name,
        quantity: p.quantity,
    ...
    const mockInventory: InventoryItem[] = mockProducts.map(p => ({
        id: p.sku,
        name: p.name,
        quantity: p.quantity,
        value: p.quantity * p.cost,
