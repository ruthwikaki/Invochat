...
describe('Product Demand Forecast Flow', () => {
    ...
    it('should forecast demand for a product with sufficient sales data', async () => {
        vi.spyOn(database, 'getHistoricalSalesForSingleSkuFromDB').mockResolvedValue(mockSalesData);
    ...
    it('should return a low confidence forecast for insufficient data', async () => {
        vi.spyOn(database, 'getHistoricalSalesForSingleSkuFromDB').mockResolvedValue(mockSalesData.slice(0, 3)); // Only 3 data points
