...
describe('Inventory Turnover Tool', () => {
  ...
  it('should return an inventory turnover report from the database', async () => {
    ...
    const result = await (getInventoryTurnoverReport as any)(input);
  ...
  it('should throw an error if the database call fails', async () => {
    ...
    await expect((getInventoryTurnoverReport as any)(input)).rejects.toThrow('An error occurred while trying to calculate the inventory turnover rate.');
  });
});
