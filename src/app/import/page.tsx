  ...
  const handleImport = async () => {
    ...
    try {
      ...
      toast({
        ...
        description:
          result.message || `Successfully imported ${result.count} products.`,
      });
        ...
        return (
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {parsedData.slice(0, 5).map((row, i) => (
                    <TableRow key={i}>
                      {headers.slice(0, 5).map(h => <TableCell key={h}>{String(row[h])}</TableCell>)}
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
            ...
            <div className="flex justify-end mt-6">
                ...
                <Button onClick={() => setCurrentStep(3)} disabled={!mappings.sku || !mappings.name || !mappings.quantity}>Review Data</Button>
            </div>
          </CardContent>
        );
      case 3:
        return (
            <CardContent>
                ...
                <div className="mt-6 flex justify-end items-center">
                 ...
                 {isImporting && <Progress value={isImporting ? undefined : 100} className="mt-4" />}
            </CardContent>
        );
      default:
        return null;
