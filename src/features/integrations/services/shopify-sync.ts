...
export async function syncProducts(integration: ShopifyIntegration, accessToken: string) {
    ...
    try {
        ...
        logError(`Successfully synced ${recordsSynced} products for ${integration.shop_name}`);
...
export async function syncOrders(integration: ShopifyIntegration, accessToken: string) {
  const supabase = getServiceRoleClient();
  ...
  try {
    ...
    if (allOrders.length === 0) {
        logError(`No orders to sync for ${integration.shop_name}`);
        await supabase.from('sync_logs').update({ status: 'completed', completed_at: new Date().toISOString(), records_synced: 0 }).eq('id', logId);
        return;
    }
    ...
    await supabase.from('sync_logs').update({ status: 'completed', completed_at: new Date().toISOString(), records_synced: recordsSynced }).eq('id', logId);
    logError(`Successfully synced ${recordsSynced} orders for ${integration.shop_name}`);
...
export async function runFullSync(integration: ShopifyIntegration, accessToken: string) {
    const supabase = getServiceRoleClient();
    try {
        await Promise.all([
