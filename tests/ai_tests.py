        ...
        try:
            ...
            if conversation_id:
                ...
                for query in test_queries:
                    ...
                    message_response = self.api.post(f"/conversations/{conversation_id}/messages", message_data)
                    
                    # If that doesn't work, try general chat endpoint
                    if message_response.status_code not in [200, 201]:
                        message_response = self.api.post("/chat", {
                            "message": query,
                    ...
                    if message_response.status_code in [200, 201]:
                        ...
                        if any(key in response_data for key in ['response', 'content', 'message', 'assistant_message']):
                            ai_responses_received += 1
                            print(f"      ✅ AI response received")
                            
                            # Check for AI components or visualizations
                            if 'component' in response_data:
                                print(f"      🎨 Component included: {response_data['component']}")
                            
                            if 'visualization' in response_data:
                                print(f"      📊 Visualization included")
                            
                            if 'component_props' in response_data:
                                print(f"      🔧 Component props included")
                        else:
        ...
        try:
            ...
            if create_response.status_code in [200, 201]:
                ...
                for message_content in test_messages:
                    message_response = self.api.post(f"/conversations/{conversation_id}/messages", {
