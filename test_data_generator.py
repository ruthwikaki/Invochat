...
class InvochatDataGenerator:
        ...
        for company in self.companies:
            for i in range(customers_per_company):
                ...
                if customer_type == "new":
                    ...
                    total_spent = random.randint(50, 500) * 100  # Convert to cents
                elif customer_type == "returning":
                    total_orders = random.randint(4, 15)
                    total_spent = random.randint(300, 2000) * 100
                elif customer_type == "loyal":
                    total_orders = random.randint(16, 50)
                    total_spent = random.randint(1500, 8000) * 100
                else:  # vip
                    total_orders = random.randint(50, 200)
                    total_spent = random.randint(5000, 25000) * 100
                ...
                customer = {
                    ...
                    "first_order_date": first_order_date.date().isoformat(),
    ...
    def generate_orders(self, customers: List[Dict], variants: List[Dict], orders_per_company: int = 300):
        ...
        platforms = ["shopify", "woocommerce", "amazon", "manual", "website", "mobile_app"]
        ...
        for company in self.companies:
            ...
            for i in range(orders_per_company):
                ...
                order = {
                    ...
                    "source_name": random.choice(["Online Store", "Mobile App", "Amazon", "eBay", "Facebook Shop"]),
                    "tags": random.sample(["express", "gift", "wholesale", "return-customer"], k=random.randint(0, 2)),
                ...
                for variant in selected_variants:
                    ...
                    line_item = {
                        ...
                        "product_name": f"Product for {variant['sku']}",
    ...
    def generate_integrations(self):
        """Generate integrations for existing companies"""
        print("🔗 Generating integrations...")
        
        integrations = []
        platforms = ["shopify", "woocommerce", "amazon_fba"]
        sync_statuses = ["syncing_products", "syncing_sales", "syncing", "success", "failed", "idle"]
        
        for company in self.companies:
            # Each company gets 1 integration
            integration_count = 1
            selected_platforms = random.sample(platforms, integration_count)
            
            for platform in selected_platforms:
                company_name = company.get("name", f"Company{company['id'][:8]}")
                
                if platform == "shopify":
                    shop_domain = f"{company_name.lower().replace(' ', '')}.myshopify.com"
                elif platform == "woocommerce":
                    shop_domain = f"{company_name.lower().replace(' ', '')}.com"
                else:
                    shop_domain = None
                ...
                integration = {
                    ...
                    "last_sync_at": fake.date_time_between(start_date="-24h", end_date="now").isoformat(),
                    "sync_status": random.choices(sync_statuses, weights=[10, 10, 15, 40, 15, 10])[0],
                    "created_at": fake.date_time_between(start_date="-1y", end_date="now").isoformat(),
        ...
        # Insert integrations
        try:
            result = self.supabase.table('integrations').insert(integrations).execute()
            print(f"   ✅ Inserted {len(integrations)} integrations")
        except Exception as e:
            print(f"   ❌ Error inserting integrations: {e}")

    def generate_conversations_and_messages(self):
        """Generate AI conversations and messages for existing users"""
        ...
        sample_queries = [
            ...
            "Show me dead stock analysis for fashion items",
            "What should I reorder for the holiday season?",
            "Which supplier has the best delivery performance?",
            "Generate sales report for this month",
            "What are my top selling products?",
            "Show inventory turnover for electronics",
            "Recommend price optimization for slow items",
            "What customers haven't ordered in 90 days?",
            "Analyze profit margins by product category"
        ]
        
        ai_responses = [
            "Here are the products with low stock levels based on sales velocity:",
            "I've analyzed your fashion inventory and found these slow-moving items:",
            "Based on seasonal trends, here are my holiday reorder recommendations:",
            "Here's a performance analysis of your suppliers with delivery metrics:",
            "I've generated your sales report with key insights:",
            "Here are your top performing products with revenue data:",
            "Your electronics inventory shows these turnover opportunities:",
            "Based on market trends, here are pricing recommendations:",
            "These customers show signs of churn and might benefit from re-engagement:",
            "Here's your profit margin analysis with category-specific insights:"
        ]
        ...
        for user in self.users:
            ...
            # Generate 2-4 conversations per user
            conv_count = random.randint(2, 4)
            ...
            for i in range(conv_count):
                conversation = {
                    ...
                    "title": f"AI Chat: {random.choice(sample_queries)[:30]}...",
                    "created_at": fake.date_time_between(start_date="-30d", end_date="now").isoformat(),
                    "last_accessed_at": fake.date_time_between(start_date="-7d", end_date="now").isoformat(),
                ...
                conv_start_time = datetime.fromisoformat(conversation["created_at"])
                
                for j in range(msg_count):
                    msg_time = conv_start_time + timedelta(minutes=j*2)
                    ...
                    if j % 2 == 0:  # User message
                        ...
                        role = "assistant"
                        component = random.choice([None, "DataTable", "Chart", "ReorderList"]) if random.random() > 0.5 else None
                    ...
                    message = {
                        ...
                        "content": content,
                        "component": component,
                        "component_props": {"data": "sample"} if component else None,
                        "visualization": {"type": "bar", "data": []} if component == "Chart" else None,
                        "confidence": confidence,
                        "assumptions": ["Current stock", "Sales data"] if role == "assistant" else None,
        ...
        try:
            result = self.supabase.table('conversations').insert(conversations).execute()
            ...
            for i in range(0, len(messages), batch_size):
                ...
                result = self.supabase.table('messages').insert(batch).execute()
            print(f"   ✅ Inserted {len(messages)} messages")
            
        except Exception as e:
            print(f"   ❌ Error inserting conversations/messages: {e}")
    ...
    def generate_all_data(self):
        ...
        try:
            ...
            # Step 8: Generate and upload integrations
            self.generate_integrations()
            
            # Step 9: Generate and upload AI conversations
            ...
            print(f"   • Platform integrations generated")
            print(f"   • AI conversations and messages generated")
